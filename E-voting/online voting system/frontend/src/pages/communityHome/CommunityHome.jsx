  ...
  const fetchElections = async () => {
      await axios.get("http://localhost:6001/elections").then((res) => {
          ...
          if (localStorage.getItem("role")==="Voter") {
            ...
            setElections(res.data.filter(election => (election.organization === localStorage.getItem("organization") && new Date(election.end) >= new Date())));

          }else if (localStorage.getItem("role")==="Community" ) {

            setElections(res.data.filter(election => (election.organization === localStorage.getItem("username") && new Date(election.end) >= new Date())));

          }
          else{
            setElections(res.data.filter(election => new Date(election.end) >= new Date()));
          }
      }
      );
  }
    ...
    <>
      ...
      <div className="community">
      {
        localStorage.getItem("role")==="Voter" ?
          <h2>
             Ongoing <span>&</span> Upcomming <span>Elections ({localStorage.getItem("organization")})</span>
          </h2>
        :
          <h2>
            Ongoing <span>&</span> Upcomming  <span>Elections</span>
          </h2>
      }
