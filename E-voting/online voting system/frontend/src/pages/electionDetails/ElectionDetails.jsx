...
const ElectionDetails = () => {
  ...
  const handleCandidate = async () => {
    ...
    await axios.post("http://localhost:6001/apply-contestant", {electionId, contestantId: localStorage.getItem("userId"), contestantName: localStorage.getItem("username"), contestantEmail: localStorage.getItem("email")}).then((res) => {
  ...
  const handleRemove = async (id) => {
    ...
    await axios.delete(`http://localhost:6001/reject-contestant/${id}`).then((res) => {
      toast.success("Candidate Rejected", { duration: 1000, icon: "🔥" });
      fetchElectionDetails();
      getCandidates();
    }).catch((err) => {
      toast.error("Rejection failed", { duration: 1000 });
    });
  };
